...
O preflight permite GET/POST/PUT/DELETE e headers, incluindo Authorization e X-Session-Request. `AllowCredentials` é permitido somente para a origem configurada, sem `AllowAnyOrigin`. O bearer oficial fica em memória por 10 minutos. Login/refresh/logout usam refresh cookie HttpOnly/Secure/SameSite=None e sessão de 30 dias com hash no PostgreSQL. A restauração acontece ao abrir a aplicação; 401 permite uma única renovação/repetição. Logout revoga a sessão no backend. Veja [sessão persistente](sessao.md) para a migration manual, CSRF, limitações de cookies de terceiros e validação. O OAuth usa somente um cookie temporário de correlação HttpOnly/Secure/SameSite=Lax, estabelecido pela navegação da janela de conexão, independente do refresh da plataforma. Render termina TLS na borda e encaminha HTTP ao container; não foi adicionado redirecionamento HTTPS interno que causaria loops. Use sempre as URLs públicas HTTPS.
