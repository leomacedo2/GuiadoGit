...
export default function ActivityPanel({ activity, classroom = false }) {
  ...
  const id = useId();
  const { months, series } = activityWindow(activity, period, technologyLimit);
  return <section className="card mb-4" aria-label="Commits por tecnologia"><div className="card-body">
    <div className="d-flex flex-wrap justify-content-between gap-3 mb-2">
      ...
      {activity && <div className="d-flex flex-wrap align-items-end gap-2">
        <TechnologyLimitSelect value={technologyLimit} onChange={setTechnologyLimit} />
        <div><label htmlFor={id} className="form-label small mb-1">Período da atividade</label><select id={id} className="form-select form-select-sm" value={period} onChange={e => setPeriod(Number(e.target.value))}>
    ...
    {!activity ? <p className="mb-0">Este snapshot ainda não possui histórico de commits. Atualize a análise pelo GitHub para gerar esse gráfico.</p> : <>
      <p className="small text-body-secondary">Tecnologias ordenadas pelo total de commits na janela de 12 meses, mantidas no filtro de 6 meses. Meses em UTC, pela data de registro do commit (committer). Os dados refletem a coleta dos snapshots, sem atualização automática.</p>
